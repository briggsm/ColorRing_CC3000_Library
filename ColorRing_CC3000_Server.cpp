...
/**************************************************************************/
/*
  ColorRing_CC3000_ClientRef implementation
*/
/**************************************************************************/

ColorRing_CC3000_ClientRef::ColorRing_CC3000_ClientRef(ColorRing_CC3000_Client* client)
  : _client(client) 
{ }

// Return true if the referenced client is connected.  This is provided for
// compatibility with Ethernet library code.
ColorRing_CC3000_ClientRef::operator bool() {
  return connected();
}
...
bool ColorRing_CC3000_ClientRef::connected(void) {
  HANDLE_NULL(_client, false);
  return _client->connected();
}

size_t ColorRing_CC3000_ClientRef::write(uint8_t c) {
  HANDLE_NULL(_client, 0);
  return _client->write(c);
}

size_t ColorRing_CC3000_ClientRef::fastrprint(const char *str) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprint(str);
}

size_t ColorRing_CC3000_ClientRef::fastrprintln(const char *str) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprintln(str);
}

size_t ColorRing_CC3000_ClientRef::fastrprint(char *str) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprint(str);
}

size_t ColorRing_CC3000_ClientRef::fastrprintln(char *str) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprintln(str);
}

size_t ColorRing_CC3000_ClientRef::fastrprint(const __FlashStringHelper *ifsh) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprint(ifsh);
}

size_t ColorRing_CC3000_ClientRef::fastrprintln(const __FlashStringHelper *ifsh) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprintln(ifsh);
}

int16_t ColorRing_CC3000_ClientRef::write(const void *buf, uint16_t len, uint32_t flags) {
  HANDLE_NULL(_client, 0);
  return _client->write(buf, len, flags);
}

int16_t ColorRing_CC3000_ClientRef::read(void *buf, uint16_t len, uint32_t flags) {
  HANDLE_NULL(_client, 0);
  return _client->read(buf, len, flags);
}

uint8_t ColorRing_CC3000_ClientRef::read(void) {
  HANDLE_NULL(_client, 0);
  return _client->read();
}

int32_t ColorRing_CC3000_ClientRef::close(void) {
  HANDLE_NULL(_client, 0);
  return _client->close();
}

uint8_t ColorRing_CC3000_ClientRef::available(void) {
  HANDLE_NULL(_client, 0);
...
/**************************************************************************/
/*
  ColorRing_CC3000_Server implementation
*/
/**************************************************************************/

// Construct a TCP server to listen on the specified port.
ColorRing_CC3000_Server::ColorRing_CC3000_Server(uint16_t port)
  : _port(port)
  , _listenSocket(-1)
{ }

// Return a reference to a client instance which has data available to read.
ColorRing_CC3000_ClientRef ColorRing_CC3000_Server::available() {
  ...
  for (int i = 0; i < MAX_SERVER_CLIENTS; ++i) {
    if (_clients[i].connected() && _clients[i].available() > 0) {
      return ColorRing_CC3000_ClientRef(&_clients[i]);
    }
  }
  ...
  return ColorRing_CC3000_ClientRef(NULL);
}

// Initialize the server and start listening for connections.
void ColorRing_CC3000_Server::begin() {
...
size_t ColorRing_CC3000_Server::write(const uint8_t *buffer, size_t size) {
  size_t written = 0;
  for (int i = 0; i < MAX_SERVER_CLIENTS; ++i) {
...
// Return the sum of bytes written to all clients.
size_t ColorRing_CC3000_Server::write(uint8_t value) {
  return write(&value, 1);
}

// Accept new connections and update the connected clients.
void ColorRing_CC3000_Server::acceptNewConnections() {
  ...
  for (int i = 0; i < MAX_SERVER_CLIENTS; ++i) {
    if (!_clients[i].connected()) {
      ...
      if (soc > -1) {
        _clients[i] = ColorRing_CC3000_Client(soc);
      }
